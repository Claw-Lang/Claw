...
def get_tests():
    ...
    for unittest in os.listdir(os.path.join(path, "claw")):
        if unittest.endswith(".claw"):
...
def test_base(test_name):
    ...
    proc = Popen([sys.executable, os.path.join(path, "src", "claw.py"),
                 "run", os.path.join(path, "tests", "claw", test_name + ".claw")],
